...
export default function FixedReturnSimulation() {
  ...
  const result = useMemo(() => {
    const chartData: FixedChartRow[] = [];

    const annualReturn = rendement / 100;
    const taxRate = imposition / 100;
    const annualInflation = inflation / 100;
    const withdrawalRate = retrait / 100;

    let capital = patrimoineInitial;

    for (let year = 0; year <= nbrAnneesEpargne; year += 1) {
      const versementsCumules = investissementsAnnuels * year;
      const interetsCumules = Math.max(capital - patrimoineInitial - versementsCumules, 0);

      chartData.push({
        annee: String(year),
        "patrimoine initial": patrimoineInitial,
        versements: versementsCumules,
        intérêts: interetsCumules,
      });
    ...
    const capitalFinal =
      (last?.["patrimoine initial"] ?? patrimoineInitial) + (last?.versements ?? 0) + (last?.intérêts ?? 0);

    const versementsCumules = investissementsAnnuels * nbrAnneesEpargne;
    const interetsCumules = Math.max(capitalFinal - patrimoineInitial - versementsCumules, 0);
    const plusValue = interetsCumules;
    const valeurNette = capitalFinal - plusValue * taxRate;
    const revenuPassifMensuel =
      (valeurNette * withdrawalRate) / 12 / Math.pow(1 + annualInflation, nbrAnneesEpargne);
      ...
      <div className="flex h-full w-full flex-col rounded-2xl bg-card md:flex-row">
        ...
        <div className="w-full p-6 md:w-3/4 md:p-10">
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-4">
            <div>
              <div className="text-sm text-muted-foreground">Valeur future</div>
              <div className="text-sidebar-primary">{formatEuros(result.capitalFinal)}€</div>
            </div>
            <div>
              <div className="text-sm text-muted-foreground">Dont plus-value</div>
              <div className="text-sidebar-primary">{formatEuros(result.plusValue)}€</div>
            </div>
            <div>
