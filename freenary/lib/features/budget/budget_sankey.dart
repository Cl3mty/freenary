...
class BudgetSankeyChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final disponibleNode = _FlowNode(label: 'Disponible', column: 1, color: green);
    nodes.addAll([revenusNode, disponibleNode]);

    if (totalRevenues > 0) {
      links.add(_FlowLink(source: revenusNode, target: disponibleNode, value: totalRevenues));
    }

    if (totalExpenses > 0) {
      final depensesNode = _FlowNode(label: 'Dépenses', column: 2, color: red);
      nodes.add(depensesNode);
      links.add(_FlowLink(source: disponibleNode, target: depensesNode, value: totalExpenses));
      ...
      for (final category in data.expenseCategories) {
        ...
        for (final item in category.items) {
          ...
          nodes.add(itemNode);
          links.add(_FlowLink(source: catNode, target: itemNode, value: item.amount));
        }
      }
    }

    if (totalInvestments > 0) {
      final investNode = _FlowNode(label: 'Investissements', column: 2, color: accent);
      nodes.add(investNode);
      links.add(_FlowLink(source: disponibleNode, target: investNode, value: totalInvestments));
  ...
  double _layout(List<_FlowNode> nodes, List<_FlowLink> links, double width) {
    ...
    final maxColumn = nodes.map((n) => n.column).reduce((a, b) => a > b ? a : b);
    final columnCount = maxColumn + 1;
    final columnGap = (width - nodeWidth) / (columnCount > 1 ? columnCount - 1 : 1);

    for (final node in nodes) {
      final outgoing = links.where((l) => l.source == node).fold<double>(0, (s, l) => s + l.value);
      final incoming = links.where((l) => l.target == node).fold<double>(0, (s, l) => s + l.value);
    ...
    for (final colNodes in columns.values) {
      ...
      final colHeight = totalValue * pixelsPerEuro + nodePadding * (colNodes.length - 1);
    ...
    for (final entry in columns.entries) {
      ...
      final totalHeight = colNodes.fold<double>(0, (s, n) => s + n.value) * pixelsPerEuro +
  ...
  void paint(Canvas canvas, Size size) {
    for (final link in links) {
      ...
      final path = Path()
        ...
        ..cubicTo(c1, sourceY - halfWidth, c2, targetY - halfWidth, targetX, targetY - halfWidth)
        ..lineTo(targetX, targetY + halfWidth)
        ..cubicTo(c2, targetY + halfWidth, c1, sourceY + halfWidth, sourceX, sourceY + halfWidth)
        ..close();

      canvas.drawPath(path, Paint()..color = link.source.color.withValues(alpha: 0.35));
    }
    ...
    for (final node in nodes) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(Rect.fromLTWH(pillX, pillY, pillW, pillH), Radius.circular(pillH / 2)),
